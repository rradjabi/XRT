...
void *msd_tx(void *handle_ptr)
{
    ...
    socket_server_init( sock, client_sock, 8889 );
    ...
    for( ;; ) {
        ...
        if( strcmp(client_message, CMD_COMPLETE) ) {
            std::cout << "             MSD: unexpected server reply: " << client_message << std::endl;
            break;
        }
    }
...
void *mpd_rx(void *handle_ptr)
{
    ...
    for( ;; ) {
	    // Receive a message from MSD server
